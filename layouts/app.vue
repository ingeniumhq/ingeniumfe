        ...
        >
          <div class="content">
            <div class="text">
              ...
              <button
                @click="requestNotificationsPermissions()"
                type="button"
                class="ok-button mx-2 button-1"
...
useHead({
  ...
  link: [{ href: "/grid/images-grid.css" }],
});
...
async function saveMessagingDeviceToken() {
  ...
  try {
    ...
    if (currentToken) {
      ...
      await UserService.updateAFCMToken({ fcm_token: currentToken }).then(
        (res) => {
...
onMounted(() => {
  ...
  onMessage(function ({ data: { title, body, icon, image } }) {
    new Notification(title, {
      ...
      actions: [{ action: "click", title: "click here" }],
      vibrate: [200, 100, 200],
    });
  });
});
